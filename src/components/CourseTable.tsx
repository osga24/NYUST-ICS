...
const CourseTable: React.FC<CourseTableProps> = ({ tableData, isLoading = false }) => {
  ...
  const getCellStyle = (isEmpty: boolean, isSelected: boolean, isExpanded: boolean) => {
    return {
      border: `1px solid ${colors.gray[200]}`,
      padding: '0.75rem',
      minWidth: '8rem',
  ...
  const getExpandableContentStyle = (isExpanded: boolean) => {
    return {
      ...(isExpanded ? { minHeight: '6rem' } : {})
    };
  };

  const getCellTextStyle = (isExpanded: boolean) => {
    return {
              ...
              >
                {row.map((cell, cellIndex) => {
                  ...
                  return (
                    <td
                      key={cellIndex}
                      style={getCellStyle(isEmpty, isSelected, isExpanded)}
